...
| API             | `api/`                                                                                                                                                                                                                | Thin routers; `api/model_loading.py` is the one place that turns preferences into a `LoadConfig` and enforces the RAM limit. Every route under `/api` is behind `require_local_auth`. Responses derive from `ApiModel` so defaulted fields are _required_ in OpenAPI (see ADR-0006). |
