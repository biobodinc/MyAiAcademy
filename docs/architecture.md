...
| Domain services | `hardware/` (detection, live metrics, benchmark), `storage/` (layout, usage, cleanup), `profile/`, `preferences/`, `skills/` (catalog, packages, graders, sandbox, evaluation, jobs, learning, academy), `commands/`, `guide/`, `audit/`, `status/`, `models/`, `chat/`, `memory/`, `knowledge/` | Plain Python; no FastAPI imports. Each has a Pydantic schema module and is unit-tested without HTTP.                                                                                                                                                                                                                                                                          |
| API             | `api/`                                                                                                                                                                                                                                                                                           | Thin routers; `api/model_loading.py` is the one place that turns preferences into a `LoadConfig` and enforces the RAM limit; `api/streaming.py` bridges blocking generators to SSE with real cancellation (ADR-0010). Every route under `/api` is behind `require_local_auth`. Responses derive from `ApiModel` so defaulted fields are _required_ in OpenAPI (see ADR-0006). |
