...
| 6 Mobile                | Auth, QR pairing, permissions, chat, status, training control                                                              | 🚧     | Host side done and tested against a real TLS socket: an opt-in HTTPS listener (off by default, audited both ways), a certificate the host mints and a device pins, a pairing invite carrying that fingerprint as a QR payload, and a refusal to accept the installation token over the network. The phone parses invites and compares fingerprints, with tests. It cannot yet connect: React Native needs a native module and a device build to pin a certificate, which cannot be verified without hardware, so the app still reports itself unpaired and says why (ADR-0015). |
...
**Not done: the phone connecting.** React Native uses the platform's TLS stack, which
rejects a self-signed certificate, and pinning one needs a native module and a development
build. Writing that without a device to run it on would mean shipping a connection that has
never been made once. The mobile app therefore parses pairing invites and compares
fingerprints — both tested, including everything a malformed QR code can be — and otherwise
still reports itself unpaired, which is the truth.
...
| §52 Device authorization        | The pairing invite carries the certificate to pin next to the code (`/api/security/pairing-invite`) |
| §55 Transport security          | TLS with a host-minted certificate (`security/host_certificate.py`); the device pins it             |
