...
| 2 Local AI              | Local model provider, chat, memory, knowledge, model management                                                            | ✅     | llama.cpp provider, curated GGUF catalog with licence gate, resumable verified downloads, streaming chat with per-user generation defaults, conversation rename, model unload, explicit memory, lexical knowledge retrieval. Real-hardware chat still to be confirmed (see README disclosures). |
...
temperature that every message uses unless a client overrides them. The CLI offers the same through `myai models`,
